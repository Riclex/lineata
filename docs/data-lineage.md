...
- `jardins-yoba-seed-production` — US$7.49M invested since 2016 in seed production (maize, massango, massambala, potato), Humpata/Chibia (Huíla) + Chivemba (Cunene), +US$5.6M expansion planned. Source 162 (Economia & Mercado, 2020-07-24). Event 137 `announcement` + event 145 `expansion` (rice production via the "Melhor Arroz" program with UFLA/Epamig/Embrapa, 100 ha from Oct 2024 — source 166, Rádio Itatiaia). Score 60. **Known-open warning:** `operational` status backed only by announcement/expansion (no completion/construction/groundbreaking/financing event) — the operational evidence is ongoing production since 2016, but no sourced completion event exists; documented, not fabricated.

...
### 196 sources in the database, each with a confidence level
...
| State | Events | Detail |
|-------|--------|--------|
| Linked to a specific source | 167 | grounded to a full-URL article or a named publisher |
...
python db/verify_invariants.py    # structural invariants (FK, score_version, change_log orphans; 52 checks)
...
Average score: **43.13 over 107 scored projects** broadened (rounded 43; FILDA-only: 43.0 over 54 scored → rounded 43, the figure cited in the published article). Banco Sol is tracked but `evidence_complete = 0` / unscored. Under v2-2026-07 (distinct-type event points + confidence-weighted evidence bonus + 17 operational-without-progress downgrades + 2026-08-04 Tier 1 cleanup + 2026-08-04 Tier 3 AIPEX/refinery/multilateral cohorts + 2026-08-10 data-integrity fixes + 2026-08-10 multilateral cohort expansion + 2026-08-11 AIPEX/PPP/multilateral cohorts + 2026-08-11 third multilateral cohort + 2026-08-11 fourth multilateral cohort + 2026-08-11 fifth multilateral cohort + 2026-08-17 FILDA 2021 cohort).

| Range | Count | % | Interpretation |
|-------|-------|---|----------------|
| 0-20 | 28 | 26.2% | No evidence of execution beyond announcement |
| 21-40 | 23 | 21.5% | Initial or partial execution |
| 41-60 | 19 | 17.8% | Operational but no traceable timeline in sources consulted |
| 61-80 | 27 | 25.2% | Substantive execution with verifiable results |
| 81-100 | 10 | 9.3% | Strong execution with documented results |
...
| Average score 43 (FILDA-only, 54 scored) / 43 (broadened, 107 scored) | calculated | projects.execution_score | n/a | formula-derived (FILDA-only avg 43,0; broadened 43,11 → rounded 43; Banco Sol unscored) |
| 108 projects tracked (107 scored) | count | projects | n/a | direct count (evidence_complete) |
| Distribution table 16/17/1/10/10 (FILDA-only) / 28/23/19/27/10 (broadened) | calculated | projects grouped by score | n/a | formula-derived |
| Sector table | calculated | projects grouped by sector | n/a | formula-derived (Agriculture 16,4; Energy 31,9; Manufacturing 67,0; Infrastructure 36,0; Technology 57,3; Multi-sector 56,1) |
| Gov vs private (54,0 vs 42,8 broadened; FILDA-only 54,0 vs 42,4) | calculated | projects by sector | n/a | derived from 107 scored obs (government = `Government` sector, 3 projects; private = other 104) |
...
|------------------|----------------------------|----------------------------|
| Average score (FILDA-only 43 / broadened 43) | `AVG(execution_score)` over `evidence_complete=1` (FILDA-only: `source_program='FILDA'`) | per-project: score ← events + `project_evidence` ← `source_id` |
| Distribution table (e.g. 19/14/5/15/9) | `score_distribution()` buckets `execution_score` into 0-20/21-40/41-60/61-80/81-100 | same per-project chain |
...
| Gov vs private (54.0 vs 42.8 broadened) | `AVG(execution_score)` where `sector='Government'` vs `sector!='Government'` (both `evidence_complete=1`) | same per-project chain |

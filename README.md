...
- **`python db/verify_invariants.py`** — structural checks that hold for any valid dataset (47 checks): audit-trail integrity, score-version stamp, award/completion guard, evidence gating, status-backed-by-progress, source-program membership. Exit non-zero on failure.
...
│   ├── verify_invariants.py   # Structural invariant verifier (47 checks; exit non-zero on failure)
